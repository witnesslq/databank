...
public class RoleAction extends ActionSupport {
    ...
    public String create(@Param(ROLE) RoleEntity role,
        ...
        if (null == role) {
            ...
            try {
                ...
                this.setJsonModel(new JsonMessageResult(ERROR, "保存角色失败，角色名不允许重复！"));
            }
        } else {
            this.setJsonModel(new JsonMessageResult(ERROR, "想干嘛？"));
        }
        return JSON;
    }
    ...
    public String update(@Param("id") Long id,
        ...
        if (null == role) {
            ...
            // 封装已授权数据源权限
            Map<String, Map<String, String>> permission = roleManager.getTablePermission(role.getPermissionList(), role.getId());

            addModelObject("method", "update");
            addModelObject(ROLE, role);
            addModelObject("permission", permission);
            addModelObject("allDataSource", dataBankSourceManager.getAll());
            return SUCCESS;
        } else if (StringUtils.isNotEmpty(role.getName())) {
            role.setPermissionList(permissionList);
            try {
                ...
                this.setJsonModel(new JsonMessageResult("更新角色成功！"));
            } catch (Exception e) {
