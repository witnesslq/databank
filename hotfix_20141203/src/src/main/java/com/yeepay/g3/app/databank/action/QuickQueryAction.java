...
public class QuickQueryAction extends ActionSupport {
    ...
    public String query(@Param("dsId") Long dsId,
        ...
        if (null == dsId) {
            ...
            try {
                ...
                if (null != lastDsId && !dataBankSourceManager.checkDS(userDTO.getUserId(), lastDsId)) {
                    ...
                    schemaList = dbHelper.loadSchema(dataBankSourceManager.get(lastDsId));
                } else if (null != dsList && dsList.size() > 0) {
                    // 没有使用过，加载第一个数据源下面的Schema
                    schemaList = dbHelper.loadSchema(dataBankSourceManager.get(dsList.first().getId()));
                } else {
                    // 没有使用过且没有任何数据源权限
                    schemaList.put("", "未指定");
                }
        ...
        try {
            ...
            // 读取用户禁止查询的表
            Map<String, List<String>> permissions = roleManager.getTablePermission(dsId, userDTO.getUserId());

            if (null == isolation) {
                isolation = IsolationTypeEnum.UR;
            }
            QueryResult queryResult = queryHelper.queryPrepare(dialect, permissions, schema, originalSql,
                    lineNum, isolation, quickMode, skipSqlCheck);
            ...
            this.setJsonModel(new JsonMessageResult(ERROR, e.getMessage()));
            EmailHelper.reportError(getRequest(), e, " SQL 查询");
        }
        return JSON;
    }
    ...
    public String down(@Param("dsId") Long dsId,
        ...
        try {
            ...
            this.setJsonModel(new JsonMessageResult(ERROR, e.getMessage()));
            EmailHelper.reportError(getRequest(), e, " 下载报错");
        }
        return JSON;
    }
