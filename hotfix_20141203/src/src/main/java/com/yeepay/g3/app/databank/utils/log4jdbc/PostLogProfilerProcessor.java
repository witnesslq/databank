 ...
 */
package com.yeepay.g3.app.databank.utils.log4jdbc;
...
*/
		...
		System.out.println("Number of sql statements:  " + totalSql);
		System.out.println("Total number of msec    :  " + totalMsec);
		if (totalMsec > 0) {
			System.out.println("Average msec/statement  :  " + totalSql / totalMsec);
		}

		int flaggedSqlStmts = flaggedSql.size();

		if (flaggedSqlStmts > 0) {
			System.out.println("Sql statements that took more than " + threshold + " msec were flagged.");
			System.out.println("Flagged sql statements              :  " + flaggedSqlStmts);
			System.out.println("Flagged sql Total number of msec    :  " + flaggedSqlTotalMsec);
			System.out.println("Flagged sql Average msec/statement  :  " + flaggedSqlTotalMsec / flaggedSqlStmts);
	 ...
	 */
			long topOffenderCount = 1000L;
			if (topOffenderCount > flaggedSqlArray.length) {
				topOffenderCount = flaggedSqlArray.length;
			}

			System.out.println("top " + topOffenderCount + " offender" + (topOffenderCount == 1 ? "" : "s") + ":");

			ProfiledSql p;

			for (int i = 0; i < topOffenderCount; i++) {
				p = (ProfiledSql) flaggedSqlArray[i];
				System.out.println(Utilities.rightJustify(execTimeSize, "" + p.getExecTime()) + " " + p.getSql());
			}
		}
	}
