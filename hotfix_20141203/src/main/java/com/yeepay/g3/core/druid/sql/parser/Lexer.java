...
import static com.yeepay.g3.core.druid.sql.parser.CharTypes.*;
import static com.yeepay.g3.core.druid.sql.parser.LayoutCharacters.EOI;
import static com.yeepay.g3.core.druid.sql.parser.Token.*;
...
public class Lexer {
	...
	public void scanVariable() {
		if (ch != '@' && ch != ':' && ch != '#' && ch != '$') {
			throw new ParserException("不合法的变量");
		}
		...
		if (mybatisFlag) {
			if (ch != '}') {
				throw new ParserException("语法错误");
			}
			++pos;
			bufPos++;
		}
	...
	public void scanIdentifier() {
		...
		if (!firstFlag) {
			throw new ParserException("不合法的标识符");
		}
