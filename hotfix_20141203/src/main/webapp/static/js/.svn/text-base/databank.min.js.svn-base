var GV = {ctxPath: "/databank-boss", imgPath: "/databank-boss/static/images", jsPath: "/databank-boss/static/js"};
    ...
    MessageBoxExt.ajax({
        ...
        checkSuccess: function (data) {
            if (data.status == "error") {
                ...
                $("#showExpalin").attr("name", data.queryTag);
                printSQLStatus(data);
            }
...
function printSQLStatus(data) {
    $("#SQL").html("<p style='margin-left:10px;margin-right:10px;'>实际执行：" + data.actualSql + "</p><p style='margin-left:10px;margin-right:10px;'>返回条数：" + data.size + "<br></p><p style='margin-left:10px;margin-right:10px;'>访表数量：" + data.tables + " 个</p><p style='margin-left:10px;margin-right:10px;'>访列数量：" + data.columns + " 个</p><p></p><p style='margin-left:10px;margin-right:10px;'>条件数量：" + data.conditions + " 个</p><p style='margin-left:10px;margin-right:10px;'>关联关系：" + data.relationships + " 个</p><p></p><p style='margin-left:10px;margin-right:10px;'>排序数量：" + data.orderBy + " 个</p><p style='margin-left:10px;margin-right:10px;'>分组数量：" + data.groupBy + " 个</p><p></p><p style='margin-left:10px;margin-right:10px;'>开始时间：" + data.startDateTime + "</p><p style='margin-left:10px;margin-right:10px;'>分析用时：" + data.analysisMilliSecond + " ms</p><p></p><p style='margin-left:10px;margin-right:10px;'>查询用时：" + data.usedMilliSecond + " ms</p><p style='margin-left:10px;margin-right:10px;'>显示用时：" + data.printMilliSecond + " ms</p><p></p><p style='margin-left:10px;margin-right:10px;'>共计用时：" + data.totleMilliSecond + " ms</p>")
}
...
function showNote() {
    ...
    $.each(noteList, function (index, element) {
        ...
        if (element.usedTimes > 1000) {
            ...
            if (element.usedTimes > 500) {
                label = "warning"
            } else {
                if (element.usedTimes > 100) {
                    label = "primary"
                } else {
                    if (element.usedTimes > 20) {
                        label = "info"
                    }
                }
            }
