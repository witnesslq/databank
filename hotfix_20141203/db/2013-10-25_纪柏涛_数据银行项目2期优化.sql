...
-- 1.笔记使用的 Schema
ALTER TABLE DATABANK.TBL_DB_NOTE ADD NOTE_SCHEMA VARCHAR (80);
CALL SYSPROC.ADMIN_CMD('REORG TABLE DATABANK.TBL_DB_NOTE');

-- 2.SQL报错信息表
DROP TABLE DATABANK.TBL_SQL_MSG;
CREATE TABLE DATABANK.TBL_SQL_MSG
(
  ID BIGINT PRIMARY KEY
  GENERATED BY DEFAULT AS IDENTITY,
  SQLSTATE VARCHAR (20) NOT NULL,
  EXPLANATION VARCHAR (2000) NOT NULL
);
CREATE UNIQUE INDEX DATABANK."SQLSTATE_index" ON DATABANK.TBL_SQL_MSG(SQLSTATE);

INSERT INTO DATABANK.TBL_SQL_MSG(SQLSTATE, EXPLANATION) VALUES
...
('01589','语句包含有冗余规范。'),
('01592','在引用 SOURCE 函数的 CREATE FUNCTION 语句中，或：输入参数的长度、精度或小数位大于源函数相应参数的长度、精度或小数位；<br>或RETURNS 或 CAST FROM 参数的长度、精度或小数位比源函数的小；或CREATE FUNCTION 语句中的 CAST FROM 参数的长度、精度或小数位比 RETURNS 参数的大；<br>运行期间可能发生截断（那时可能会引起错误）。'),
...
('02000','发生下述异常之一：<br>SELECT INTO 语句或 INSERT 语句的子查询的结果为空表。<br>在搜索的 UPDATE 或 DELETE 语句内标识的行数为零。<br>在 FETCH 语句中引用的游标位置处于结果表最后一行之后。'),
('02501','游标位置对于当前行的 FETCH 无效。'),
('02502','检测到删除或更新孔'),
('02506','遇到了错误，并且已按 RETURN DATA UNTIL 子句指定那样容许该错误。'),
...
('38552','SYSFUN 模式（由 IBM 提供）中的函数已异常终止。<br>可以在消息正文中找到下列原因码之一：<br>01 数字值超出范围<br>02 被零除<br>03 算术溢出或下溢<br>04 日期格式无效<br>05 时间格式无效<br>06 时间戳记格式无效<br>07 时间戳记持续时间的字符表示法无效<br>08 时间间隔类型无效（必须是 1、2、4、8、16、32、64、128、256 之一）<br>09 字符串太长<br>10 字符串函数中的长度或位置超出范围<br>11 浮点数的字符表示法无效'),
...
('42501','授权标识不具有对标识对象执行指定操作的特权。请联系系统部相关人员添加数据银行对该标识符的访问权限。'),
...
('42817','由于某个视图或约束依赖于该列，并且指定了 RESTRICT，或者由于该列包含在分区键中，或者由于该列是安全标号列，所以无法废弃该列。'),
...
('428E6','用户定义的谓词中的方法的搜索自变量与索引扩展名的相应搜索方法的搜索自变量不匹配。'),
...
('42989','使用表达式生成的列或者安全标号列不能在 BEFORE 触发器中使用。'),
...
('55039','不允许访问或状态过渡，因为该表空间未处于适当状态。'),
...
('56033','长串列的插入值或更新值必须是主机变量或 NULL。'),
('56038','此环境不支持请求的功能部件。'),
...
('57011','虚拟存储器或数据库资源不可用。是不是日志太多了，找 DBA 升级表空间吧〜'),
...
('ZZZZZ','占位符 sqlstate 仅供开发使用。在交付代码之前必须对其进行更改。');


-- 3.记录更加详细的查询状态码
-- 5.下载行数
ALTER TABLE DATABANK.TBL_DB_LOG ADD LOG_SQL_STATUS VARCHAR (20) DEFAULT '00000';
ALTER TABLE DATABANK.TBL_DB_LOG ADD LOG_SCHEMA VARCHAR (100);
ALTER TABLE DATABANK.TBL_DB_LOG ADD LOG_SIZE BIGINT WITH DEFAULT 0;
CALL SYSPROC.ADMIN_CMD('REORG TABLE DATABANK.TBL_DB_LOG');


-- 4.功能、菜单
INSERT INTO EMP.TBL_FUNCTION (FUNCTIONID, FUNCTIONNAME, FUNCTIONURL, PREFUNCTIONID, FUNCTIONSTATUS, DESCRIPTION, RISKLEVEL, FUNCTIONTYPE, CREATETIME, CHECKFUNCTIONID, CHECKNEEDED, LOGNEEDED) VALUES (-330042, '[数据银行]出错码查询', 'https://boss3g.yeepay.com/databank-boss/query/error.action', -99910033, 'ACTIVE', '查看DB2出错码', 'MIDDLE', 'DEPARTMENT_PRIVATE', CURRENT_TIMESTAMP, null, null, null);
INSERT INTO EMP.TBL_ROLEANDFUNCTIONRELATION (ROLEID, FUNCTIONID) values (152,-330042);
...
-- 角色功能关系
INSERT INTO EMP.TBL_ROLEANDFUNCTIONRELATION (ROLEID, FUNCTIONID) values
(162, -330001),
(162, -330042);

-- 菜单
INSERT INTO EMP.TBL_MENU (MENUID, MENUNAME, FUNCTIONID, SEQUENCE, PARENTID, LEVELNUM, ICONNAME) VALUES
(-330010, '出错码查询', -330042, 10, -3301, 3, null);
